...
public class ActionsContract implements SQLDataTypeDefines {
    ...
    public void queryInsertActionsData(HashMap<String, TableColumn> actionsMap, Context c){
        ...
        for(String key : actionsMap.keySet()){
            if(actionsMap.get(key).getClass() == TableStringColumn.class) {
                content.put(key, ((TableStringColumn) actionsMap.get(key)).getValue());
            } else if(actionsMap.get(key).getClass() == TableIntegerColumn.class){
                content.put(key, ((TableIntegerColumn) actionsMap.get(key)).getValue());
            }
