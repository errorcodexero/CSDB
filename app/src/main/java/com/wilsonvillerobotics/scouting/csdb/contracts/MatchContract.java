...
public class MatchContract implements SQLDataTypeDefines {
    ...
    public void queryInsertMatchData(HashMap<String, TableColumn> matchMap, Context c){
        ...
        for(String key : matchMap.keySet()){
            if(matchMap.get(key).getClass() == TableStringColumn.class) {
                content.put(key, ((TableStringColumn) matchMap.get(key)).getValue());
            } else if(matchMap.get(key).getClass() == TableIntegerColumn.class){
                content.put(key, ((TableIntegerColumn) matchMap.get(key)).getValue());
            }
