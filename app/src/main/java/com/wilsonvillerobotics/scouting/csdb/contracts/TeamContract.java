...
import com.wilsonvillerobotics.scouting.csdb.R;
...
public class TeamContract implements SQLDataTypeDefines {
    ...
    public void queryInsertTeamData(HashMap<String, TableColumn> teamMap, Context c) {
        ...
        for(String key : teamMap.keySet()) {
            if(teamMap.get(key).getClass() ==  TableStringColumn.class) {
                content.put(key, ((TableStringColumn) teamMap.get(key)).getValue());
            } else if(teamMap.get(key).getClass() == TableIntegerColumn.class){
                content.put(key, ((TableIntegerColumn) teamMap.get(key)).getValue());
            }
